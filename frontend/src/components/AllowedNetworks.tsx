...
const AllowedNetworks: React.FC = () => {
  ...
  const handleAddNetwork = () => {
    ...
    if (!validateNetwork(trimmed)) {
      setError(
        'Invalid network format. Use IP (192.168.1.1), CIDR (192.168.1.0/24), or hostname (mail.example.com)',
      );
      return;
    }
  ...
  const handleCancelEdit = () => {
    ...
    setNewNetwork('');
    setError(null);
    void fetchNetworks(); // Reload original data
  };
      ...
      {successMessage && (
        <div className="bg-green-500/20 border border-green-500/50 text-green-300 p-4 rounded-md mb-4">
          ...
          <p className="text-sm mt-1">
            Run <code className="bg-gray-900/50 px-2 py-1 rounded">sudo postfix reload</code> to
            apply changes.
          </p>
        </div>
      )}
