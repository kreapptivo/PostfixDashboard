...
const parseLogLine = (line) => {
  ...
  const [, hostname, process, _pid, message] = parts;
...
const readAndParseLogs = async () => {
  ...
  try {
    ...
    const logFiles = files
      .filter((f) => f.startsWith(config.postfix.logPrefix))
      .sort()
      .reverse();
...
app.get('/api/volume-trends', authenticate, async (req, res) => {
  try {
    ...
    const sortedVolume = Object.values(volumeByDay).sort(
    ...
    recentLogs.forEach((log, index) => {
      ...
      if (line.includes('relay access denied')) {
        activities.push({
          ...
          timestamp: log.timestamp,
          type: 'security',
          description: `Relay access denied for a client.`,
        });
        ...
        activities.push({
          ...
          timestamp: log.timestamp,
          type: 'system',
          description: 'Postfix service was stopped or terminated.',
        });
        ...
        activities.push({
          ...
          timestamp: log.timestamp,
          type: 'system',
          description: 'Postfix service started.',
        });
      }
    });
...
app.get('/api/allowed-networks', authenticate, async (req, res) => {
  try {
    ...
    const mynetworksLine = lines.find(
      (line) => line.trim().startsWith('mynetworks') && !line.trim().startsWith('#'),
    );
    if (mynetworksLine) {
    ...
    res
      .status(500)
      .json({ error: `Could not read Postfix config file at ${config.postfix.configPath}` });
  }
});
...
app.post('/api/allowed-networks', authenticate, async (req, res) => {
  try {
    ...
    const validNetworks = networks.filter((net) => {
      ...
      return (
        trimmed && (ipRegex.test(trimmed) || hostnameRegex.test(trimmed) || ipv6Regex.test(trimmed))
      );
    });
...
const extractRelayIP = (line) => {
  ...
  const connectMatch = line.match(/connect from [^[]*\[(\d{1,3}(?:\.\d{1,3}){3})\]/);
  if (connectMatch) return connectMatch[1];

  // PRIORITY 2: Look for "client=" - this is also the ORIGINAL sender
  const clientMatch = line.match(/client=[^[]*\[(\d{1,3}(?:\.\d{1,3}){3})\]/);
  ...
  try {
    ...
    const topIPs = Object.entries(ipCounts)
      .map(([ip, counts]) => ({
        ...
        rejected: counts.rejected,
        successRate:
          counts.messages > 0 ? ((counts.sent / counts.messages) * 100).toFixed(1) : '0.0',
